...
def main():
    ...
    conf = Config(**load_yml(config))
    tmp_dir = conf.imagery_params.tmp
    # print(tmp_dir)
    ...
    for input_shp in tqdm.tqdm(shps):
        ...
        try:
            result = subprocess.run(
                ["python", "fao_models/ForestClassifierBeam.py", "--input", str(input_shp_path), "--output", str(out_shp), "--model-config", str(config)],
                check=True,
                capture_output=True,
                text=True
            )
